...
class SkymatesStudyScreen extends StatefulWidget {
  @override
  _SkymatesStudyScreenState createState() => _SkymatesStudyScreenState();
}

class _SkymatesStudyScreenState extends State<SkymatesStudyScreen> {
  final String skymatesUrl =
      'https://skymates.com/student/international-student-document-submission-form/';
  ...
  Future<void> _launchURL() async {
    final Uri url = Uri.parse(skymatesUrl);
    if (!await launchUrl(url, mode: LaunchMode.externalApplication)) {
      throw 'Could not launch $skymatesUrl';
    }
  }
  ...
  Future<void> _uploadDataToFirestore() async {
    ...
    if (user != null) {
      try {
        // Upload the I20 number and name to the 'skymates' collection
        ...
        // Upload the school name to the 'users' collection
        await FirebaseFirestore.instance
            .collection('users')
            .doc(user.email)
            .set(
          {
            'أكاديمية التدريب العملي': 'Sky Mates',
          },
          SetOptions(merge: true),
        );

        // Navigate to the next screen if both uploads are successful
        Navigator.push(
          context,
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              SizedBox(height: 30),
              ElevatedButton(
                onPressed: (_isI20Valid && _isNameValid)
                ...
                style: ElevatedButton.styleFrom(
                  backgroundColor:
                      (_isI20Valid && _isNameValid) ? Color(0xFF1591B2) : Colors.grey,
                  padding: EdgeInsets.symmetric(vertical: 15, horizontal: 40),
                  shape: RoundedRectangleBorder(
                    borderRadius: BorderRadius.circular(30),
                  ),
